    ...
    return {
      ...
      children: (
        <Tabs
          ...
          defaultActiveKey="1"
          style={{ height: 500 }}
          items={cumRap.cumRapChieu.map((rap, index) => {
            return {
              key: index,
              label: (
                <div className="cumRap__title">
                  <h2>{rap.tenCumRap}</h2>
                </div>
              ),
              children: (
                <div className="cumRap__schedules grid grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-1 gap-4">
                  {rap.lichChieuPhim.map((phim, index) => {
                    console.log("phim", phim);
                    return (
                      <button className="schedule__item" key={index}>
                        <NavLink
                          to={`/booking/${phim.maLichChieu}`}
